...
export interface PaginationProps { page: number; pageCount: number; onChange: (page: number) => void; /** Tổng số bản ghi, để hiện "1–20 / 240". */ total?: number; pageSize?: number }
...
export function Pagination({ page, pageCount, onChange, total, pageSize }: PaginationProps) {
  ...
  return (
    <nav className="eb-pager" aria-label="Phân trang">
      {from && <span className="eb-pager__info">{from}–{to} / {total}</span>}
      <div className="eb-pager__pages">
        <button className="eb-pager__btn" onClick={() => onChange(page - 1)} disabled={page <= 1} aria-label="Trang trước"><Chev dir="l" /></button>
        ...
        <button className="eb-pager__btn" onClick={() => onChange(page + 1)} disabled={page >= pageCount} aria-label="Trang sau"><Chev dir="r" /></button>
      </div>
    </nav>
  );
}
