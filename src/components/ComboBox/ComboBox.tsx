...
export interface ComboBoxProps {
  ...
  /** Allow values that aren't in the list. */
  allowCustom?: boolean;
  size?: 'sm' | 'md' | 'lg'; disabled?: boolean;
}
/** A searchable Select. Use above 15 options, or when users recall a name faster than they scroll. */
export function ComboBox({ label, hint, error, placeholder = 'Type to search…', options, selected, defaultSelected, onChange, allowCustom, size = 'md', disabled }: ComboBoxProps) {
  ...
  return (
    <ACombo className={cls} selectedKey={selected} defaultSelectedKey={defaultSelected} onSelectionChange={(k: Key | null) => onChange?.(k == null ? null : String(k))} allowsCustomValue={allowCustom} isDisabled={disabled} isInvalid={!!error} menuTrigger="focus">
      ...
      <div className="eb-field__control eb-combo__control">
        ...
        <Button className="eb-combo__btn" aria-label="Open the list"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 6l4 4 4-4" /></svg></Button>
      </div>
      ...
      <Popover className="eb-popover" offset={4} style={{ width: 'var(--trigger-width)' }}>
        <ListBox className="eb-list" renderEmptyState={() => <div className="eb-combo__empty">No results</div>}>
          {options.map((o) => (
